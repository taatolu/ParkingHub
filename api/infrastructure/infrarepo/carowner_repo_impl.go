...
func (r *CarOwnerRepositoryImpl) Delete(id uint) error {
	return r.DB.Transaction(func(tx *gorm.DB) error {
		...
		countResult := tx.Model(&model.CarOwner{}).Where("id = ?", id).Count(&count)
		...
		if err := tx.Where("owner_id = ?", id).Delete(&model.Car{}).Error; err != nil {
			return fmt.Errorf("Ownerに紐づくCarの削除に失敗: %w", err)
		}

		//Ownerが存在する場合、削除を実行
		result := tx.Delete(&model.CarOwner{}, id)
		if result.Error != nil {
