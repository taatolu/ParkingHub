...
func TestCarOwnerRepositoryImpl_Delete(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.testname, func(t *testing.T) {
			...
			//gormでテスト用テーブル作成
			if err := db.AutoMigrate(&model.CarOwner{}, &model.Car{}); err != nil {
				t.Fatalf("テスト用のテーブルの作成失敗")
			}
			//上記テーブルにデータ投入
...
func TestCarOwnerRepositoryImpl_Delete_SoftDeleteRelatedCars(t *testing.T) {
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	if err != nil {
		t.Fatalf("sqliteの初期化に失敗")
	}
	...
	car := &model.Car{
		...
		SerialNumber1:       12,
		SerialNumber2:       34,
		ShakenExpiration:    time.Now().AddDate(1, 0, 0),
		InsuranceExpiration: time.Now().AddDate(1, 0, 0),
		OwnerID:             owner.ID,
	}
	if err := db.Create(car).Error; err != nil {
		t.Fatalf("Carデータ投入に失敗")
	}

	repo := &CarOwnerRepositoryImpl{DB: db}
	deleteErr := repo.Delete(owner.ID)
	assert.NoError(t, deleteErr)

	var activeCarCount int64
	db.Model(&model.Car{}).Where("owner_id = ?", owner.ID).Count(&activeCarCount)
	assert.Equal(t, int64(0), activeCarCount, "Ownerに紐づくCarがソフトデリートされていません")

	var deletedCarCount int64
	db.Unscoped().Model(&model.Car{}).Where("owner_id = ? AND deleted_at IS NOT NULL", owner.ID).Count(&deletedCarCount)
	assert.Equal(t, int64(1), deletedCarCount, "Ownerに紐づくCarのDeletedAtが更新されていません")
}
